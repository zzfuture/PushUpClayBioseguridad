...
namespace API.Controllers
{
    public class TipoPersonaController : BaseController
    {
        ...
        [HttpGet]
        public async Task<IEnumerable<TipoPersonaDto>> GetTipoPersonas()
        {
            var result = await _unitOfWork.TipoPersonas.GetAllAsync();
            return _mapper.Map<IEnumerable<TipoPersonaDto>>(result);
        }
        [HttpGet("{id}")]
        public async Task<TipoPersonaDto> GetTipoPersonasById(int id)
        {
